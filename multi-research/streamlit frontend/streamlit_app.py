...
if start_btn:
    with st.spinner("🤖 Agents are researching your topic..."):
        ...
        try:
            ...
            # Extract the actual report from the formatter node
            if "formatter" in raw_result and isinstance(raw_result["formatter"], dict):
                extracted_result = raw_result["formatter"]
            else:
            ...
            st.error("Research timed out after 10 minutes. Try a simpler topic.")
        except Exception as e:
            st.error(f"Error: {str(e)}")

# Display Research Results
if st.session_state.research_completed and st.session_state.research_results:
    raw_result = st.session_state.research_results.get("raw", {})
    result = st.session_state.research_results.get("extracted", {})
    ...
    with c1:
        st.metric("Quality Score", f"{quality.get('overall_score', 'N/A')}/10")
    with c2:
        st.metric("Questions", len(result.get("research_questions", [])))
    with c3:
        st.metric("Sources", len(result.get("search_results", [])))
    with c4:
        st.metric("Iterations", result.get("iteration_count", metadata.get("total_steps", 0)))
    ...
    with tab1:
        # Try multiple locations for the report
        report = (
            result.get("formatted_output") or
            result.get("draft") or
            result.get("summarized_content") or
            raw_result.get("formatted_output") or
            raw_result.get("draft")
        )
        ...
        if report and len(report) > 100:
            ...
            with col2:
                st.download_button(
                    ...
                    data=json.dumps(raw_result, indent=2),
                    file_name=f"{topic.replace(' ', '_')}_report.json",
                    mime="application/json"
                )
        else:
            st.warning(f"No report content available. Report length: {len(report) if report else 0}")
            with st.expander("Debug: Raw Response"):
                st.json(raw_result)
    
    with tab2:
        # Summary
        summary = result.get("summarized_content") or raw_result.get("summarized_content")
        if summary:
        ...
        # Metadata
        if metadata:
            with st.expander("⚙️ Research Metadata"):
                st.json(metadata)
